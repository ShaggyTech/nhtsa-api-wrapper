...
import { DecodeVinValuesExtended } from '../'

// Mocks
import { createMockResponse } from '@vitest/helpers'
import { mockResults } from '@vitest/data'

describe('api/endpoints/DecodeVinValuesExtended.ts', () => {
...
describe('DecodeVinValuesExtended', () => {
  ...
  it('returns a Promise', () => {
    const response = DecodeVinValuesExtended('test')
  ...
  it('fetches data when provided VIN only', async () => {
    const results = await DecodeVinValuesExtended(vin)
  ...
  it('fetches data when provided VIN and doFetch = true', async () => {
    const results = await DecodeVinValuesExtended(vin, true)
  ...
  it('fetches data when provided VIN and params', async () => {
    const results = await DecodeVinValuesExtended(vin, { ...params })
  ...
  it('fetches data when provided VIN, params, and doFetch = true', async () => {
    const results = await DecodeVinValuesExtended(vin, { ...params }, true)
  ...
  it('accepts params.modelYear as string', async () => {
    const results = await DecodeVinValuesExtended(
      vin,
      { modelYear: '2018' },
      true
    )
  ...
  it('returns url string when provided VIN and doFetch = false', async () => {
    const results = await DecodeVinValuesExtended(vin, false)
  ...
  it('returns url string when provided VIN, params, and doFetch = false', async () => {
    const results = await DecodeVinValuesExtended(vin, { ...params }, false)
  ...
  it('rejects with error if vin is undefined', async () => {
    await expect(() =>
      DecodeVinValuesExtended(undefined as unknown as string)
  ...
  it.each([1234, ['a', 'b'], { a: 'b' }, null])(
    ...
    async (arg) => {
      await expect(() =>
        DecodeVinValuesExtended(arg as unknown as string)
      ).rejects.toThrowError(/error validating argument named "vin"/)
  ...
  it.each([1234, ['a', 'b'], null])(
    ...
    async (arg) => {
      await expect(() =>
        DecodeVinValuesExtended(vin, arg as unknown as object)
