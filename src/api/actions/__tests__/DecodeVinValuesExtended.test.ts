...
describe('NHTSA.DecodeVin()', () => {
  test('it decodes a VIN', async () => {
    const client = new DecodeVinValuesExtended();
  ...
  test('it returns an Error when no VIN argument is provided', async () => {
    ...
    expect(response).toEqual(
      Error(
        'DecodeVinValuesExtended, vin argument is required and must be a string, got: undefined'
      )
    );
  });
  ...
  test('it returns an Error when invalid typeof VIN argument is provided', async () => {
    const client = new DecodeVinValuesExtended();
    const response = await client
      .DecodeVinValuesExtended(3929343 as any)
      .catch(err => err);

    expect(mockCrossFetch).toHaveBeenCalledTimes(0);
    expect(response).toEqual(
      Error(
        'DecodeVinValuesExtended, vin argument is required and must be a string, got: 3929343'
      )
    );
  });
